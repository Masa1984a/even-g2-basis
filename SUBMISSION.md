...
| `*-alpha.png` | 上記それぞれの背景透過版 |

倍率はニアレストネイバーの整数倍なのでにじまない。

透過版は黒い画素を透明にしたもの。グラスのディスプレイは発光式で黒い画素は光っておらず
素通しなので、透過の方が実機の見え方に近い。色は最大輝度で固定して明るさをアルファで
表しているため、どんな背景に重ねても加算的な発光として自然に見える。スライドや記事など
背景色が黒でないところに置くときはこちらを使う。

**ストア提出には透過でない方を使う。** 掲載面の背景が不明なため。
